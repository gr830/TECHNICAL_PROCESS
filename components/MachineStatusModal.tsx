...
const MachineStatusModal: React.FC<MachineStatusModalProps> = ({ onClose, millingMachines, turningMachines, onUpdateMilling, onUpdateTurning }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const type = isMilling ? 'MillingMachines' : 'TurningMachines';
    try {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/95 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-6xl max-h-[94vh] rounded-[2.5rem] flex flex-col shadow-2xl overflow-hidden ring-1 ring-white/5">
        
        {/* Фиксированная шапка */}
        <div className="shrink-0 z-20">
          <div className="px-10 py-6 border-b border-slate-800 flex justify-between items-center bg-slate-900">
          ...
          <div className="px-10 py-4 flex flex-wrap gap-6 items-center justify-between border-b border-slate-800 bg-slate-900/80 backdrop-blur-md">
            <div className="flex bg-slate-950 p-1.5 rounded-2xl border border-slate-800 shadow-inner">
              <button onClick={() => setActiveTab('milling')} className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'milling' ? 'bg-orange-600 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>Фрезерные</button>
              <button onClick={() => setActiveTab('turning')} className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'turning' ? 'bg-orange-600 text-white shadow-xl' : 'text-slate-500 hover:text-slate-300'}`}>Токарные</button>
            </div>
            <div className="flex gap-4">
              ...
              <button onClick={fetchData} className="px-8 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-black uppercase rounded-xl shadow-2xl flex items-center gap-3 transition-all active:scale-95 border border-indigo-400/20">
                {loading ? <div className="w-4 h-4 border-2 border-white/20 border-t-white rounded-full animate-spin"></div> : 'Получить данные'}
              </button>
            </div>
          </div>
        </div>

        {/* Область контента с единым скроллом */}
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-950/10">
          <div className="p-10 grid grid-cols-1 lg:grid-cols-12 gap-12 min-h-full items-start">
            ...
            <div className="lg:col-span-5 flex flex-col bg-slate-950/30 rounded-[2.5rem] border border-slate-800/50 p-8 shadow-2xl h-fit">
               <div className="flex items-center justify-between mb-8 shrink-0">
                  <div className="flex flex-col">
                    <label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em]">Результат подбора</label>
                    <span className="text-[10px] text-slate-600 font-bold uppercase mt-1">Found machines</span>
                  </div>
               ...
               <div className="space-y-5">
                 {filteredMachines.map((m, idx) => (
                   <div key={idx} className="bg-slate-900/60 border border-slate-800 p-6 rounded-[1.5rem] hover:border-orange-500/50 transition-all group shadow-sm animate-in fade-in slide-in-from-bottom-2 duration-300">
                     <div className="flex justify-between items-start mb-4">
                       <div>
                         <h4 className="font-black text-white group-hover:text-orange-400 transition-colors uppercase text-lg tracking-tight">{m["Станок"]}</h4>
                         <div className="flex gap-2 mt-1">
                           <span className="text-[8px] bg-slate-800 px-2 py-0.5 rounded text-indigo-400 font-bold uppercase">Ready</span>
                           <span className="text-[8px] bg-slate-800 px-2 py-0.5 rounded text-emerald-400 font-bold uppercase">Sync OK</span>
                         </div>
                       </div>
                       <span className="text-[10px] bg-slate-800 px-3 py-1 rounded-lg text-slate-400 font-mono font-bold border border-slate-700">
                         {isMilling ? (m as MillingMachineData)["Сложность (G)"] : (m as TurningMachineData)["Группа сложн. (G)"]}
                       </span>
                     </div>
                     <div className="grid grid-cols-2 gap-x-8 gap-y-3">
                       <div className="flex flex-col">
                         <span className="text-[9px] text-slate-600 font-black uppercase mb-1">Точность</span>
                         <span className="text-xs text-slate-200 font-bold">
                           {isMilling ? (m as MillingMachineData)["Точность (GA)"] : `X:${(m as TurningMachineData)["Точность по X (GAX)"]} Z:${(m as TurningMachineData)["Точность по Z (GAZ)"]}`}
                         </span>
                       </div>
                       <div className="flex flex-col">
                         <span className="text-[9px] text-slate-600 font-black uppercase mb-1">Магазин</span>
                         <span className="text-xs text-slate-200 font-bold">
                           {isMilling ? (m as MillingMachineData)["Инстр. (T)"] : (m as TurningMachineData)["Кол-во инстр. (T)"]}
                         </span>
                       </div>
                       <div className="flex flex-col col-span-2 border-t border-slate-800 pt-3">
                         <span className="text-[9px] text-slate-600 font-black uppercase mb-1">Оснащение</span>
                         <span className="text-xs text-orange-400/90 font-bold italic">
                           {isMilling ? (m as MillingMachineData)["Оснастка"] : (m as TurningMachineData)["Группа оснастки"]}
                         </span>
                       </div>
                       <div className="flex flex-col col-span-2 border-t border-slate-800 pt-3">
                         <span className="text-[9px] text-slate-600 font-black uppercase mb-1">Рабочая зона</span>
                         <span className="text-xs text-slate-400 font-bold">
                           {isMilling ? (m as MillingMachineData)["Габариты (AxBxC / DxL)"] : (m as TurningMachineData)["Габариты (DxL)"]}
                         </span>
                       </div>
                     </div>
                   </div>
                 ...
                 {!loading && filteredMachines.length === 0 && (
                   <div className="py-24 text-center flex flex-col items-center gap-6">
                     <div className="w-20 h-20 bg-slate-900 rounded-full flex items-center justify-center text-slate-700 ring-4 ring-slate-800/50">
                       <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                     </div>
                     <div>
